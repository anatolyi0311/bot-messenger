...
// downloadTranscription - это метод, который извлекает из базы данных список голосовых сообщений, которые находятся на этапе "WAIT" и готовы для загрузки расшифровки текста для голосового сообщения, которое было успешно обработано в Salute. Для каждого такого сообщения он выполняет загрузку расшифровки текста с помощью API Salute и обновляет статус в базе данных, а также отправляет пользователю сообщение об успешной загрузке или ошибке при загрузке расшифровки текста.
func (w *Worker) downloadTranscription() {
	...
	// Проход по каждому голосовому сообщению, готовому для загрузки расшифровки текста, и выполнение процесса загрузки расшифровки текста с помощью API Salute. Если статус обработки еще не "DONE", то продолжаем ожидать, иначе обновляем статус в базе данных и отправляем пользователю сообщение об успешной загрузке или ошибке при загрузке расшифровки текста.
	for i := range downloadTranscriptionData {
		...
		// Обновление статуса обработки голосового сообщения в базе данных на "WAIT" и сохранение расшифровки текста для данного голосового сообщения.
...
// createSummary - это метод, который извлекает из базы данных список голосовых сообщений, которые находятся на этапе "DOWNLOAD" и готовы для создания краткого содержания. Для каждого такого сообщения он выполняет создание краткого содержания с помощью GigaChat и обновляет статус обработки в базе данных, а также отправляет пользователю сообщение об успешном создании краткого содержания или ошибке при создании краткого содержания.
func (w *Worker) createSummary() {
	...
	for i := range createSumData {
		// Получение краткого содержания для голосового сообщения с помощью GigaChat. Если при создании краткого содержания произошла ошибка, то отправляем пользователю сообщение об ошибке и обновляем статус в базе данных на "FAIL", иначе обновляем статус на "SUCCESS" и сохраняем краткое содержание для данного голосового сообщения.
